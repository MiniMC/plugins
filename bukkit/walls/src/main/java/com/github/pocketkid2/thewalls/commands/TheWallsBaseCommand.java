...
public class TheWallsBaseCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		if (args.length <= 0) {
			...
			sender.sendMessage(ChatColor.GRAY + "This server is running " + ChatColor.GOLD + pdf.getName() + ChatColor.GRAY + " version " + ChatColor.GOLD + pdf.getVersion());
			sender.sendMessage(ChatColor.GRAY + "There are " + ChatColor.YELLOW + plugin.getGM().getArenas().size() + ChatColor.GRAY + " arenas, with " + ChatColor.YELLOW
					+ plugin.getGM().getArenas().stream().filter(a -> a.getStatus() == Status.INGAME).collect(Collectors.toList()).size() + ChatColor.GRAY + " currently in-game");
			sender.sendMessage(ChatColor.GRAY + "Use " + ChatColor.DARK_GREEN + "/" + label + " help" + ChatColor.GRAY + " to see a list of commands");
			return true;
		}
		...
		public void execute(CommandSender sender, String label, String[] args) {
			if (args.length > 0) {
				for (TheWallsSubCommand sub : subCommands) {
					for (String name : sub.names()) {
						if (name.equalsIgnoreCase(args[0])) {
							sender.sendMessage(plugin.addPrefix(ChatColor.GRAY + "That command is used as follows: " + ChatColor.GOLD + "/" + label + " " + sub.usageMessage()));
						}
					}
				}
			} else {
				sender.sendMessage(plugin.addPrefix(ChatColor.GRAY + "The following commands are available:"));
				for (TheWallsSubCommand sub : subCommands) {
					if (sub.isAdminCommand() && sender.hasPermission("thewalls.admin")) {
