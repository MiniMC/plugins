...
import com.rammelkast.anticheatreloaded.BansMySQL;
			...
			public void run() {
				...
				for (String event : actions) {
					...
					if (event.startsWith("COMMAND[")) {
						...
						try {
							PreparedStatement ps = BansMySQL.getConnection().prepareStatement("INSERT IGNORE INTO Punishments (name,uuid,reason,operator,punishmentType,start,end) VALUES (?,?,?,?,?,?,?)");
							ps.setString(1, user.getPlayer().getName());
							ps.setString(2, user.getPlayer().getUniqueId().toString().replace("-", ""));
							ps.setString(3, "Banned for cheating!");
							ps.setString(4, "Anticheat");
							ps.setString(5, "BAN");
							ps.setString(6, Long.toString(Instant.now().getEpochSecond()));
							ps.setString(7, "-1");
							ps.executeUpdate();
						} catch (Exception e) {
						...
						user.getPlayer().kickPlayer(ChatColor.translateAlternateColorCodes('&', "Banned for cheating!"));
